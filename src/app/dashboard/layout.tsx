...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    const primaryColorClass = isPatient ? 'flowcare-primary' : 'bg-flowcare-accent';
    const activeClass = isPatient ? 'sidebar-active-patient' : 'sidebar-active-provider';

    return (
        <div className="flex h-screen bg-gray-100 font-sans">
            {/* Sidebar */}
            <aside className={`w-64 bg-white shadow-lg flex flex-col ${isPatient ? 'border-t-4 border-flowcare-primary' : 'border-t-4 border-flowcare-accent'}`}>
